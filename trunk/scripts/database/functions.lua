...
function P.open(dbfile)
    ...
    if xword then
        if not dbfile then
            dbfile = P.getdbfilename()
        end
        xword.makeDirs(dbfile)
    elseif not dbfile then
        return -- Can't figure out the directory, so we have to return
    end
...
function P.find_one(...)
    ...
    assert(#ret <= 1, "Not a unique match ["..tostring(#ret).." matche(s)]")
    return ret[1]
end
...
function P.update(options)
    ...
    print("UPDATE puz_index SET".." "..table.concat(set, ', ').." "..where)
