...
BEGIN_EVENT_TABLE(GridCtrl, wxWindow)
...
static const int MAX_POINT_SIZE = 150;
static const int MIN_POINT_SIZE = 2;
...
GridCtrl::GridCtrl(wxWindow * parent)
    : wxWindow(parent, wxID_ANY, wxDefaultPosition, wxDefaultSize, wxFULL_REPAINT_ON_RESIZE | wxWANTS_CHARS),
      m_grid(NULL),
      m_borderSize(1),
      m_boxSize(20),         // Have to initialize this to something for the font scaling
      m_focusedSquare(NULL),
      m_focusedStart(NULL),
      m_focusedEnd(NULL),
      m_direction(DIR_ACROSS),
      m_style(DEFAULT_GRID_STYLE),
...
void
GridCtrl::SetPuzzle(XPuzzle * puz)
{
    m_puz  = puz;
    m_grid = &puz->m_grid;
...
{
    // Keep focus on this window or else bad things happen
...
GridCtrl::DrawGrid(wxDC & dc, const wxRegion & updateRegion)
{
    ...
    // If we do have an update region, redraw all squares within the region
        ...
        // Just deal with the update rect, not region . . . it's easier and faster
        wxRect rect = updateRegion.GetBox();

        wxLogDebug(_T("Update rect (%d, %d, %d, %d"), rect.x, rect.y, rect.width, rect.height);
        wxLogDebug(_T("Rect: (%d, %d, %d, %d)"), GetRect().x, GetRect().y, GetRect().width, GetRect().height);
        // Offset the update rect by the top left corner of this window's rectangle
        // I don't think this should be necessary, but occasionally
        //    when the frame is activated, the update rect coordinates are off
        //    by exactly this amount
        //rect.Offset(GetRect().GetTopLeft());

        int firstCol = (rect.GetLeft()   - m_rect.GetLeft()) / GetSquareSize();
        int firstRow = (rect.GetTop()    - m_rect.GetTop())  / GetSquareSize();
        int lastCol  = (rect.GetRight()  - m_rect.GetLeft()) / GetSquareSize();
        int lastRow  = (rect.GetBottom() - m_rect.GetTop())  / GetSquareSize();
        ...
        wxLogDebug(_T("Drawing squares from (%d, %d) to (%d, %d)"), firstCol, firstRow, lastCol, lastRow);
...
GridCtrl::DrawSquare(wxDC & dc, const XSquare & square, const wxColour & color)
{
    ...
    if (square.text != '-') {
        ...
        dc.DrawText((wxChar)square.text, x + (m_boxSize - width)/2, y + (m_boxSize - height));
    }
...
GridCtrl::ChangeSquareFocus(XSquare * square, bool direction)
{
    ...
    XSquare * oldStart   = oldFocused == NULL ? NULL : oldFocused->WordStart(m_direction);
    XSquare * oldEnd     = oldFocused == NULL ? NULL : oldFocused->WordEnd(m_direction);
    ...
    // Otherwise redraw everything
        ...
        if (oldStart != NULL && oldEnd != NULL)
            for (square = oldStart; square != NULL; square = square->Next(direction)) {
                DrawSquare(dc, *square, m_colors[WHITE_SQUARE]);
                if (square == oldEnd)
                    break;
            }
        // Draw new word
        for (square = m_focusedStart; square != NULL; square = square->Next(direction)) {
...
GridCtrl::SetFocusedClue(int cluenum, bool direction)
{
    ...
    if (square != NULL) {
        if (HasStyle(BLANK_ON_NEW_WORD))
            SetIfExists(square, FindSquare(square->WordStart(direction), FIND_BLANK_SQUARE, direction));
...
{
    if (! m_grid || m_grid->GetWidth() <= 0 || m_grid->GetHeight() <= 0)
        return wxDefaultSize;

    // Scale the grid (uses several member variables so that the info carries over for all functions)
    int boxSize = std::min( (m_rect.width  - (m_grid->GetWidth() + 1)  * m_borderSize) / m_grid->GetWidth(),
                            (m_rect.height - (m_grid->GetHeight() + 1) * m_borderSize) / m_grid->GetHeight() );

    return wxSize( m_grid->GetWidth()  * (boxSize + m_borderSize) + m_borderSize,
                   m_grid->GetHeight() * (boxSize + m_borderSize) + m_borderSize);
}
...
GridCtrl::Scale()
{
    ...
    m_boxSize = std::min( (max_width  - (m_grid->GetWidth() + 1)  * m_borderSize) / m_grid->GetWidth(),
                          (max_height - (m_grid->GetHeight() + 1) * m_borderSize) / m_grid->GetHeight() );

    m_rect.SetWidth (m_grid->GetWidth()  * (m_boxSize + m_borderSize) + m_borderSize);
    m_rect.SetHeight(m_grid->GetHeight() * (m_boxSize + m_borderSize) + m_borderSize);

    // Horizontal align (X coordinate)
    if ((m_align & wxALIGN_RIGHT) != 0)
        m_rect.SetX(max_width - m_rect.width);
    else if (m_align & wxALIGN_CENTER_HORIZONTAL)
        m_rect.SetX( (max_width  - m_rect.width)  / 2 );
    else
        m_rect.SetX(0);

    // Vertical align (Y coordinate)
    if ( (m_align & wxALIGN_BOTTOM) != 0)
        m_rect.SetY(max_height - m_rect.height);
...
GridCtrl::OnKeyDown(wxKeyEvent & evt)
{
    ...
    if      (key == WXK_LEFT  || key == WXK_NUMPAD_LEFT)     /* Left      */   OnArrow(DIR_ACROSS, FIND_PREV, mod);
    else if (key == WXK_RIGHT || key == WXK_NUMPAD_RIGHT)    /* Right     */   OnArrow(DIR_ACROSS, FIND_NEXT, mod);
    else if (key == WXK_UP    || key == WXK_NUMPAD_UP)       /* Up        */   OnArrow(DIR_DOWN,   FIND_PREV, mod);
    else if (key == WXK_DOWN  || key == WXK_NUMPAD_DOWN)     /* Down      */   OnArrow(DIR_DOWN,   FIND_NEXT, mod);
    else if (key == WXK_HOME  || key == WXK_NUMPAD_HOME)     /* Home      */   OnHome(mod);
    else if (key == WXK_END   || key == WXK_NUMPAD_END)      /* End       */   OnEnd(mod);
    else if (key == WXK_TAB   || key == WXK_NUMPAD_TAB       /* Tab       */
          || key == WXK_RETURN || key == WXK_NUMPAD_ENTER)   /* Enter     */   OnTab(mod);
    else if (65 <= key && key <= 90 || key == WXK_SPACE)     /* Letter    */   OnLetter(key, mod);
    else if (key == WXK_BACK)                                /* Backspace */   OnBackspace(mod);
    else if (key == WXK_DELETE || key == WXK_NUMPAD_DELETE)  /* Delete    */   OnDelete(mod);
}
...
GridCtrl::OnLetter(char key, int mod)
{
    ...
    if (HasStyle(MOVE_AFTER_LETTER)) {
        if (HasStyle(MOVE_TO_NEXT_BLANK))
            newSquare = FindSquare(m_focusedSquare, FIND_BLANK_SQUARE, m_direction, FIND_NEXT);
        else
            newSquare = FindNextSquare(m_focusedSquare, FIND_WHITE_SQUARE, m_direction, FIND_NEXT);
    }
...
GridCtrl::OnArrow(bool arrowDirection, bool increment, int mod)
{
    ...
    if (mod == wxMOD_SHIFT) {
        if (m_direction == arrowDirection) {
            newSquare = FindNextSquare(m_focusedSquare, FIND_WORD(arrowDirection, m_focusedSquare), arrowDirection, increment, SKIP_BLACK_SQUARES, WRAP_LINES);
            if (newSquare != NULL)
                newSquare = newSquare->WordStart(arrowDirection);
        }
        else
            newSquare = FindNextSquare(m_focusedSquare, FIND_WHITE_SQUARE, arrowDirection, increment, SKIP_BLACK_SQUARES);

        // Test BLANK_ON_NEW_WORD
        if (HasStyle(BLANK_ON_NEW_WORD) && newSquare != NULL && newSquare->WordStart(m_direction) != m_focusedSquare->WordStart(m_direction))
            SetIfExists(newSquare, FindSquare(newSquare->WordStart(m_direction), FIND_BLANK_SQUARE, m_direction, FIND_NEXT));
    }
    else {  // No Shift
        if (m_direction != arrowDirection && HasStyle(PAUSE_ON_SWITCH) && m_focusedSquare->IsBlank())
            SetSquareFocus(m_focusedSquare, ! m_direction);   // shortcut
        else
            newSquare = FindNextSquare(m_focusedSquare, FIND_WHITE_SQUARE, arrowDirection, increment, SKIP_BLACK_SQUARES);
    }

    SetSquareFocus(newSquare, (mod == wxMOD_SHIFT ? m_direction : arrowDirection));
}
...
GridCtrl::OnBackspace(int mod)
{
    ...
    SetSquareFocus(FindNextSquare(m_focusedSquare, FIND_WHITE_SQUARE, m_direction, FIND_PREV), m_direction);
}
...
GridCtrl::OnHome(int mod)
{
    if (mod == wxMOD_CMD) {
        ...
        if (HasStyle(BLANK_ON_NEW_WORD))
            SetSquareFocus( EitherOr(newSquare, FindSquare(newSquare, FIND_BLANK_SQUARE, m_direction)), m_direction );
        else
            SetSquareFocus(newSquare, m_direction);
    }
    else
...
GridCtrl::OnTab(int mod)
{
    ...
    if (mod == wxMOD_SHIFT) {
        newSquare = FindNextSquare(m_focusedStart, FIND_CLUE(m_direction, m_focusedStart), DIR_ACROSS, FIND_PREV, SKIP_BLACK_SQUARES, WRAP_LINES);
        if (newSquare == NULL) {
            // Find the last clue
            newDir = !newDir;
            newSquare = FindNextSquare(m_grid->LastWhite(), FIND_CLUE(newDir, m_grid->FirstWhite()), DIR_ACROSS, FIND_PREV, SKIP_BLACK_SQUARES, WRAP_LINES);
        }
    }
    else {
        newSquare = FindNextSquare(m_focusedStart, FIND_CLUE(m_direction, m_focusedStart), DIR_ACROSS, FIND_NEXT, SKIP_BLACK_SQUARES, WRAP_LINES);
        if (newSquare == NULL) {
            newDir = !newDir;
            newSquare = m_grid->FirstWhite();  // always the first clue for both directions
        }
    }
    newSquare = newSquare->WordStart(newDir);

    // Test BLANK_ON_NEW_WORD
    if (HasStyle(BLANK_ON_NEW_WORD) && newSquare->WordStart(newDir) != m_focusedSquare->WordStart(m_direction))
        SetIfExists(newSquare, FindSquare(newSquare, FIND_BLANK_SQUARE, newDir, FIND_NEXT));
...
GridCtrl::HitTest(int x, int y)
{
    ...
    if (0 <= col && col < m_grid->GetWidth() && 0 <= row && row < m_grid->GetHeight())
        return &At(col, row);
    return NULL;
}
...
{
    XSquare * square;
    for (square = m_grid->FirstWhite(); square != NULL; square = square->Next())
